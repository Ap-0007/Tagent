            ...
            <header className="px-6 py-5 border-b border-zinc-800/60">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-zinc-500 mt-0.5">{loading ? "Loading..." : `${incidents.length} live incidents`}</p>
                    </div>
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-lg overflow-hidden">
                    {incidents.length === 0 && !loading ? (
                        <div className="px-6 py-12 text-center text-sm text-zinc-500">
                            {error ? "Start API Gateway and Discovery Service to derive live incidents." : "No active incidents found in current cluster data."}
                        </div>
                    ) : (
                            ...
                            <tbody className="divide-y divide-zinc-800/50">
                                {incidents.map((i) => (
                                    <tr key={i.id} className="hover:bg-zinc-800/20 transition-colors">
                                        ...
                                        <td className="px-3 py-3 text-zinc-200 font-medium">
                                            <Link href={`/incidents/${i.id}`} className="hover:text-emerald-400">{i.title}</Link>
                                        </td>
